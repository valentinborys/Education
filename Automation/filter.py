...
print("Якого кольору автомобіль бажаєте?")
color_avto = str(input())

filter_avto = []

for marka, info in car_data.items():
    color, year, engine, type, price = info

    if color == color_avto:
        filter_avto.append((marka, info))

print(f"Вывести цвет авто: {color}$")

for car, details in filter_avto:
